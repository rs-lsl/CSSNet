...
def generate_data(ratio=3):
    ...
    original_hs = original_hs[:, :int(original_hs.shape[1] - original_hs.shape[1] % 48),
                  :int(original_hs.shape[2] - original_hs.shape[2] % 48)]

    original_pan = np.float32(np.load(pan_path) / 4095.0)  # [:, :1000*12, :1000*12]
    original_pan = original_pan[:, :int(original_pan.shape[1] - original_pan.shape[1] % (48 * 3)),
                   :int(original_pan.shape[2] - original_pan.shape[2] % (48 * 3))]
...
def unsupervise_crop():
    ...
    for i2 in range(img_num):
        ...
        pan_data.append(np.expand_dims(
            np.float32(np.load('/home/aistudio/data/data95831/pan_ziyuan.npy')[:1000 * 12, :1000 * 12]) / 4095.0,
            axis=0))
