...
class attention(nn.Module):
    def __init__(self, ks=3, mid_ch=64):
        ...
        self.conv0 = nn.Sequential(
            nn.Conv2d(mid_ch, mid_ch, ks, padding=int(ks / 2), padding_mode='circular'),
            nn.LeakyReLU(),
            nn.Conv2d(mid_ch, mid_ch, ks, padding=int(ks / 2), padding_mode='circular'))

        self.spe_att = nn.Sequential(
            nn.Conv2d(mid_ch, int(mid_ch / 2), 1),
            nn.LeakyReLU(),
            nn.Conv2d(int(mid_ch / 2), mid_ch, 1),
            nn.Sigmoid()
        )
...
# please modify the 'output_pad' and 'padding' parameters to change the output's size
class cross_scale_attention(nn.Module):
    def __init__(self, in_ch, band_hs, shifts, ks=5, mid_ch=64, ratio=4, stride=4, softmax_scale=10):
        ...
        self.in_ch = in_ch
        self.output_pad = 0 # if (ratio % 2) == 1 else 0
        self.shifts = shifts
...
class Our_net(nn.Module):
    def __init__(self, band_hs, band_ms, mid_ch=10, ratio=16):  # band is the number of hyperspectral image
        ...
        # self.encoder_hs_net_red = encoder_hs(band_hs, ks=3, ratio=ratio, len_res=5, mid_channel=mid_ch)
        # self.encoder_hs_net_red2 = encoder_hs(band_hs, ks=3, ratio=ratio, len_res=5, mid_channel=mid_ch)
        # self.encoder_hs_net_red3 = encoder_hs(band_hs, ks=3, ratio=ratio, len_res=5, mid_channel=mid_ch)
        ...
        unfold_size = 3
        self.transformer = cross_scale_attention(ks=unfold_size, in_ch=mid_ch, shifts=[0, 0], band_hs=band_hs, mid_ch=mid_ch, ratio=ratio,
        ...
        self.transformer_red3 = cross_scale_attention(ks=unfold_size, in_ch=mid_ch, shifts=[1, 1],band_hs=band_hs, mid_ch=mid_ch, ratio=ratio,
                                                      stride=3, softmax_scale=10)
        # low_transformer = cross_scale_attention(ks=3, in_ch=band_ms, band_hs=band_hs, mid_ch=mid_ch, ratio=ratio, softmax_scale=10)
        ...
        self.down_dim = nn.Sequential(
            nn.Conv2d(5 * band_hs, 5*band_hs, kernel_size=11, padding=5, padding_mode=padding_mode, groups=5 * band_hs),
            nn.Conv2d(5 * band_hs, band_hs, kernel_size=1))
