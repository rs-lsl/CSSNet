...
from ours.ournet_structure_red_828 import Our_net  # 还有 Pfnet_structure2
...
from loss_function import cc_loss, SSIMLoss
...
def high_level(hs, high_hs, batch_size, band_hs, win_size, h, w):
    ...
    for i in range(batch_size):
        ...
        sorted_hs.append(torch.Tensor(np.transpose(hs0[i, :, sort_index[i, :]])[None, ...]))
...
def ournet(train_ms_image, train_pan_image, train_label,
    ...
    # please modify this part into the torch version
    step = [300]
    print('setp', step)
    optimizer = optim.Adam(lr=learning_rate, params=model.parameters())
    scheduler0 = optim.lr_scheduler.MultiStepLR(optimizer, step, gamma=0.2)

    # for the hp fusion task, it's better to add the LinearWarmup
    # scheduler = optim.lr_scheduler.LinearWarmup(
    #     learning_rate=scheduler0, warmup_steps=10, start_lr=1e-7, end_lr=learning_rate, verbose=False)  # warmup_steps为epoches

    # rescale_value = 10   # 减少语义类别数目和复杂度
    ...
    for epoch in range(num_epochs):
        ...
        for i, (hs, ms, labels) in enumerate(train_loader):
            ...
            hs_var = high_level(hs, high_hs, batch_size, band_hs, 16, h, w)
            ms_var = high_level(ms, high_ms, batch_size, band_ms, 32, H, W)
            ssim_loss_term = ssim_loss(labels, result_hs)
            hs_loss = Pixel_loss(result_hs, labels)
            # print(torch.log(hs_var), torch.log(ms_var))

            loss = hs_loss + ssim_loss_term*0.1 + hs_var*0.1 + ms_var*0.1 #+ bicubic_down_loss*0.1 + bicubic_up_loss*0.1
    ...
    # torch.save(model.state_dict(), '/home/aistudio/result/parameters/ournet_hp_'+name+'.pth')

    # 测试模型
    # model_dict =torch.load('/home/aistudio/result/parameters/ournet_hp_'+name+'.pth')    # 加载训练好的模型参数
    ...
    with torch.no_grad():
        ...
        a = torch.concat(image_all, dim=0)
