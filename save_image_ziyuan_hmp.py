...
def generate_data(ratio_hs = 12):
    ...
    Downsampler12 = Downsampler(12, padding=3)

    hs_path = '/home/aistudio/data/data95831/HSI.npy'
    ms_path = '/home/aistudio/data/data95831/MSI.npy'
    pan_path = '/home/aistudio/data/data95831/PAN.npy'
    ...
    original_ms = np.float32(np.load(ms_path) / 4095.0)
    # print(original_hs.shape)
    original_ms = original_ms[:, :int(original_ms.shape[1]-original_ms.shape[1]%(patch_size*ratio_ms)), :int(original_ms.shape[2]-original_ms.shape[2]%(patch_size*ratio_ms))]
 
    original_pan = np.float32(np.load(pan_path) / 4095.0)[None, ...]
    # print(original_pan.shape)
    original_pan = original_pan[:, :int(original_pan.shape[1]-original_pan.shape[1]%(patch_size*ratio_pan)), :int(original_pan.shape[2]-original_pan.shape[2]%(patch_size*ratio_pan))]

    # np.save(save_dir+'/hs_30m.npy', original_hs)
    # np.save(save_dir+'/ms_10m.npy', original_pan)
    print(original_hs.shape)
    print(original_ms.shape)
    print(original_pan.shape)
        
    temp_blur_hs = Downsampler12(torch.tensor(np.expand_dims(original_hs, axis=0)))
    print(temp_blur_hs.shape)
    temp_blur_hs = np.squeeze(temp_blur_hs.numpy(), axis=0)

    temp_blur_ms = Downsampler12(torch.tensor(np.expand_dims(original_ms, axis=0)))
    print(temp_blur_ms.shape)
    temp_blur_ms = np.squeeze(temp_blur_ms.numpy(), axis=0)

    temp_blur_pan = Downsampler12(torch.tensor(np.expand_dims(original_pan, axis=0)))
    ...
    return temp_blur_hs, temp_blur_ms, temp_blur_pan, original_hs

def generate_data_high():
    ...
    hs_path = '/home/aistudio/data/data95831/HS_ziyuan.npy'
    pan_path = '/home/aistudio/data/data95831/pan_ziyuan.npy'
    # save_dir = "/home/aistudio/work/ziyuan_hm"

    original_hs = np.float32(np.load(hs_path) / 4095.0)#[:, :200, :200]
    # print(original_hs.shape)
    original_hs = original_hs[:, :int(original_hs.shape[1]-original_hs.shape[1]%12), :int(original_hs.shape[2]-original_hs.shape[2]%12)]
 
    original_pan = np.float32(np.load(pan_path) / 4095.0)[None, ...]#[:, :200*12, :200*12]
    # print(original_pan.shape)
    original_pan = original_pan[:, :int(original_pan.shape[1]-original_pan.shape[1]%(12*12)), :int(original_pan.shape[2]-original_pan.shape[2]%(12*12))]

    # np.save(save_dir+'/hs_90m.npy', temp_blur_hs)
    # np.save(save_dir+'/ms_30m.npy', temp_blur_pan)
    return original_hs, original_pan

def crop_data(hs_data, ms_data, pan_data, label_data, training_size=64, step_facter=2):
    train_ratio = 0.8
    train_factor = 1
    ...
    train_hs_image, train_pan_image, train_label = \
        Crop_traindata_four(hs_data,             # 可调整size
                            ms_data,
                            pan_data,
                            label_data,
                            training_size,
                            test=False, step_facter=step_facter, ratio=int(pan_data.shape[2]/hs_data.shape[2]))
...
def Crop_traindata_four(image_hs, image_ms, image_pan, image_label, size, test=False, step_facter=1, ratio_ms=3, ratio_pan=12):
    image_hs_all = []
    image_ms_all = []
    image_pan_all = []
    label = []
    ...
    for j in range(0, image_hs.shape[1] - size + 1, int(size/step_facter)):
        for k in range(0, image_hs.shape[2] - size + 1, int(size/step_facter)):
            temp_image_hs = image_hs[:, j :j + size, k :k + size]
            temp_image_ms = image_ms[:, j*ratio_ms :(j + size)*ratio_ms, k*ratio_ms :(k + size)*ratio_ms]
            temp_image_pan = image_pan[:, j*ratio_pan :(j + size)*ratio_pan, k*ratio_pan :(k + size)*ratio_pan]
            temp_label = image_label[:, j*ratio_pan :(j + size)*ratio_pan, k*ratio_pan :(k + size)*ratio_pan]
    ...
    print("size of lrhsimage:{}".
            format(image_hs_all.shape))
    ...
    return image_hs_all, image_ms_all, image_pan_all, image_label_all
